...
export default function Workspace() {
  ...
  useEffect(() => {
    if (!pages.length) {
      ...
      setLastSavedBlocksHash("");
      setSaveStatus("idle");
      return;
    }

    const currentPage =
      (selectedPageId ? pages.find((page) => page.id === selectedPageId) : null) ?? pages[0];
    if (!currentPage) return;

    const normalized = normalizeBlocks(currentPage.content);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      mutateContent({ pageId: selectedPageKey, content });
    }, 1200);

    return () => clearTimeout(timer);
  }, [editorBlocks, lastSavedBlocksHash, selectedPage?.id, mutateContent]);
